...
from .retrieval.biobert_qa import BioBertQA
...
def compute_recall_at_k(question: str, used_chunks: List[str], k: int = EVAL_K) -> float:
    relevant_ids, retrieved_top_k, hits = _compute_relevance_stats(question, used_chunks, k)
    if not relevant_ids:
        return 0.0
...
def compute_precision_at_k(question: str, used_chunks: List[str], k: int = EVAL_K) -> float:
    relevant_ids, retrieved_top_k, hits = _compute_relevance_stats(question, used_chunks, k)
    if not retrieved_top_k:
        return 0.0
    precision = hits / len(retrieved_top_k)
    return round(precision, 6)


def compute_f1_at_k(question: str, used_chunks: List[str], k: int = EVAL_K) -> float:
    precision = compute_precision_at_k(question, used_chunks, k)
    recall = compute_recall_at_k(question, used_chunks, k)
...
def _compute_relevance_stats(question: str, used_chunks: List[str], k: int):
    ...
    return relevant_ids, retrieved_top_k, hits
    ...
    with file_lock:
        ...
        with open(EVAL_RESULTS_PATH, "a", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=[
                ...
                "fallback_triggered", "precision_at_k", "recall_at_k", "f1_at_k",
                "refusal_accuracy", "latency_ms", "k"
            ])
...
try:
    ...
    try:
        biobert_model = BioBertQA(model_name=config.BIOBERT_MODEL)
        logger.info("BioBERT QA model initialized successfully.")
    except Exception as e:
        biobert_model = None
        logger.warning("BioBERT QA model unavailable, continuing without it: %s", e)

    try:
        rag_model = RAG(model_name=config.RAG_MODEL)
        logger.info("RAG model initialized successfully.")
    except Exception as e:
        rag_model = None
        logger.warning("RAG model unavailable, continuing without it: %s", e)
...
def ask_question(req: AskRequest):
    ...
    try:
        ...
        qa_result: QAResult = qa_engine.ask(
            question,
        ...
        precision_at_k = compute_precision_at_k(question, used_chunks, EVAL_K)
        recall_at_k = compute_recall_at_k(question, used_chunks, EVAL_K)
        f1_at_k = compute_f1_at_k(question, used_chunks, EVAL_K)
        latency_ms = (time.perf_counter() - start_time) * 1000.0
        expected_refusal = any(keyword in question.lower() for keyword in [
            "do i have", "am i sick", "am i at risk", "could i have",
            "is it cancer", "do i have cancer", "should i be worried", "do i have tumor"
        ])
        predicted_refusal = method == "safe-response" or "I don't have enough information" in answer
        refusal_accuracy = float(predicted_refusal == expected_refusal)
