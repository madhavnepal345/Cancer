...
class EmbeddingGenerator:
    def __init__(self, model_name: str = None):
        ...
        self.dimension = 384
        try:
            model_ref = model_name or config.EMBEDDING_MODEL
            self.model = SentenceTransformer(model_ref)
            self.dimension = self.model.get_sentence_embedding_dimension()
        except Exception as e:
            print(f"Falling back to local hashing embeddings: {e}")
            self.fallback = True
...
def build_and_save_faiss_checkpoint(
    ...
    if not chunks:
        print("No chunks found. Writing an empty FAISS index and chunk file.")
